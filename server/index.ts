...
app.get("/authed/playlist", async (c) => {
  ...
  const API = SpotifyApi.withAccessToken(process.env.SPOTIFY_CLIENT_ID!,{access_token: user.spotify_token, refresh_token: user.spotify_refresh_token, expires_in: user.spotify_token_iat.getTime(), token_type: "Bearer"} );
  ...
  const searchYoutube = async (search: string) =>{
    const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(search)}&type=video&key=${process.env.GOOGLE_KEY}`)
    const data = await response.json();
    const youtubeId= data.items[0].snippet.id.videoId;
    return youtubeId;
    }
  const downloadVideo = async (url: string) =>{
    const download = await fetch("https://cobalt.tools/api",{ method:"POST",
      headers: {Accept: "application/json",
        "Content-Type": "application/json"},
      body: JSON.stringify({url:url,audioFormat: "wav", downloadMode: "audio"
      })
    })
    return download;
  }
